...
# The ENTIRE import must be in ONE PL/SQL block because wwv_flow.g_flow_id
# is a package variable that only persists within a single session call.
# Splitting into multiple execute() calls resets the package state.
...
begin
    ...
    wwv_imp_workspace.create_flow(
        ...
        p_logo_type=>'T',
        p_logo_text=>nvl(wwv_flow_application_install.get_application_name,'TEST APP'),
    ...
    wwv_flow_imp_page.create_page(
        ...
        p_name=>'Home',
        p_alias=>'HOME',
        p_step_title=>nvl(wwv_flow_application_install.get_application_name,'TEST APP'),
        p_autocomplete_on_off=>'OFF',
    ...
    wwv_flow_imp_page.create_page(
        ...
        p_name=>'Login Page',
        p_alias=>'LOGIN',
        p_step_title=>nvl(wwv_flow_application_install.get_application_name,'TEST APP')||' - Log In',
    ...
    wwv_flow_imp_page.create_page_plug(
        ...
        p_plug_name=>nvl(wwv_flow_application_install.get_application_name,'TEST APP'),
...
print("Executing complete import in single PL/SQL block...")
print(f"SQL length: {len(sql)} characters")

try:
    cursor.execute(sql)
    conn.commit()
    print("\n✅ Import block executed successfully!")
...
row = cursor.fetchone()
if row:
    print(f"✅ App created: {row[0]} - {row[1]} ({row[2]})")
else:
