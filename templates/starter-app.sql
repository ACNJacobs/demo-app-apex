...
--------------------------------------------------------------------------------
-- Oracle APEX 26.1 Minimal Starter App Template
-- 
-- This template creates a minimal runnable APEX app with:
--   - Home page (blank, ready for your content)
--   - Login page (Oracle APEX Accounts authentication)
--   - Universal Theme 42
--   - Minimal navigation (Home only)
--
-- IMPORTANT: This file uses hardcoded IDs from the original SCAFF APP export.
-- It will ONLY work if imported into a workspace where these IDs do not already
-- exist. For a fresh workspace, this works out of the box via SQLcl.
--
-- For importing into a workspace with existing apps, use the Python script:
--   python scripts/import_starter_app.py --id 200 --name "MY APP" --alias "MY-APP"
--
-- Usage via SQLcl (preserves package state between statements):
--   sql -S SAH/password@host:port/PDB @starter-app.sql
--------------------------------------------------------------------------------
...
prompt APPLICATION &APP_ID. - &APP_NAME.
